...
6, If sub-agents are available then delegate in granular tasks to as many sub-tasks as the provider allows. This allows you to simply relax and run "sleep(sweet glorious rest milliseconds)" ;) plus finding single points of failures, maintaining context limits, and similar basic developer coencepts.
...
### 2.4 Mid-Work Commit Cadence, Git, And Some Wisdom

Commit frequently with meainginful commit messages **at every meaningful milestone**, not only at TODO completion:

- Frequent commits are a great means to a unique or backup changelog end → commit is the general mantra; look how jj and similar new source control ideaologies operate
- Model behavior analytics can be applied to both in-progress, or previously written, code logic to maintain a high standard or catch lesser quality code → "I just now realized that that this xor comparison is done manually in multiple functions, I should write a grand overarching function to handle them all or find an industry standard library to use! A good senior developer and orchestrator thinks ten-steps ahead."
- A new file/module is working in isolation → commit mentioning what this class or code can do which will be seen when a new agent session starts working again
- A test suite is written, added to, or passing with full green for the first time → commit the milestone
- A refactor is complete and tests still pass → commit, if hopelessly broken requiring longer runs in a new sessions with clean context then simply make a noted broken commit or place into a temporary branch
- Before any risky or wide-reaching change → commit (checkpoint)
- Working on the last idea: what are the smallest sized chunks of logic this problem or need can be broken down into? Incorporate appropriate programming paradigms → Before committing anything resembling a monolith, that the project lead or president will see and scream "That's a MEGAlith that only an 'Arch, BTW' neck beard could concoct!' think about what programming paradigms might aide the project further down the road or in code maintainability such as SIIP, and object or functional programming paradigm.
...
### 2.5 Document And Build Workflows For Other Agent Sessions

Create the means for not only the code, or headers, to be able to be read easily enough to be self-documenting, but such that other agent or orchestrator sessions (read: "developers") can easily understand what the code does even in such a basic concept as directory structurea and file names, in additoon to likely getting a glimpse into how it is used, how to use it for similar logic if a need for it arrises within context, or related needs. Write schemas, create thorough docs with hyperlinks to other related docs, replace instances of similar logic with one unified means of handling both via a call with parameters or aguments: work done at the 4000 LOC stage will have compound-**ED**, and inheritly comopund-**ing** savings futher down the development pipeline and allow the developer to acheive more with the same amount of resources spend. 

- Write appropriate opencode/kilo cli SKILLS.md or AGENTS.md documents as needed when appropriate
- Document anything that either might need to be known by the end-user, a future development chat session or cycle, or may be ambigious: whether within code as comments to future agent work sessions or within docs to the user or future end-users.
- File names and directory structure can do a lot → Reading a file from src/utils_and_helpers/textual_operations/char_case_checker.cpp is a lot clearer than src/alphanum-chars.cpp
...
- [ ] All new public functions/methods have doc-comments → think how linux kernel style documentation is done within headers
- [ ] No function exceeds 40 lines (extract if needed)
- [ ] No file exceeds 200 lines (split if needed)
- [ ] Tests exist for every new behaviour (unit + at least one integration test per feature)
- [ ] `git log --oneline` shows ≥ 2 granular commits for this task
- [ ] No `console.log` / debug prints left in production paths, use proper command-line option(s) to enable them, and even a third party library implementation to keep it simple and clean
- [ ] Linter/formatter passes with zero warnings → if you canno't use one that is useful then inform the user to install it on their system or yourself if you have the appropriate rights (inform user if you need to be added to `/etc/sudoers`)
